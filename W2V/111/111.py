...
# 数据加载和预处理（修改为读取文件夹下所有txt文件）
def load_and_preprocess_data(folder_path):
    """加载文件夹中所有txt文件并预处理文本数据"""
    sentences = []
    # 检查文件夹是否存在
    ...
    if len(txt_files) == 0:
        raise ValueError(f"文件夹 {folder_path} 中未找到任何txt文件")

    print(f"找到 {len(txt_files)} 个txt文件，开始加载...")

    # 遍历所有txt文件
    for file_idx, filename in enumerate(txt_files):
        file_path = os.path.join(folder_path, filename)
        try:
            # 优先用utf-8编码，失败则用gbk（中文常见编码）
        ...
        # 显示加载进度
        if (file_idx + 1) % 10 == 0:
            print(f"已加载 {file_idx + 1}/{len(txt_files)} 个文件")

    return sentences


# 加载数据（修改为文件夹路径）
folder_path = r"e:\shi yan dai ma\Word2vec\JournalArticle2013_2023"
sentences = load_and_preprocess_data(folder_path)
print(f"\n总共加载了 {len(sentences)} 条文本数据")
print("前5条数据示例:")
...
# Word2Vec模型(Skip-gram with Negative Sampling)（保持不变）
