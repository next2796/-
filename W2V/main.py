...
def load_pretrained_model(model_path):
    ...
    print(f"预训练模型加载完成，词汇表大小: {len(model.key_to_index)}")
    return model
...
def test_pretrained_model(model):
    ...
    # 测试词语相似性
    print("\n1. 词语相似性测试：")
    test_words = ['king', 'queen', 'man', 'woman', 'China', 'America']
    ...
    analogy_tasks = [
        ...
        ('China', 'Beijing', 'Japan'),  # China - Beijing + Tokyo = Japan
        ('Paris', 'France', 'London')  # Paris - France + England = London
    ]
    
    for task in analogy_tasks:
        try:
            result = model.most_similar(positive=[task[2], task[0]], negative=[task[1]], topn=1)
            print(f"{task[0]} : {task[1]} :: {task[2]} : {result[0][0]} (相似度: {result[0][1]:.3f})")
        except KeyError as e:
            print(f"类比推理失败，词语不在词汇表中: {e}")
...
# 测试训练后的模型
def test_trained_model(w2v_model):
    """测试训练后的模型的词语相似性和类比推理"""
    print("\n" + "="*60)
    print("训练后模型测试")
    print("="*60)
    ...
    # 测试词汇相似度计算
    print("\n2. 词汇相似度计算：")
    word_pairs = [('曹操', '刘备'), ('诸葛亮', '周瑜'), ('关羽', '张飞'), ('吕布', '董卓')]
...
if __name__ == "__main__":
    # 1. 加载预训练模型
    pretrained_model_path = "./archive/GoogleNews-vectors-negative300.bin"
    pretrained_model = load_pretrained_model(pretrained_model_path)
    
    # 2. 测试预训练模型
    test_pretrained_model(pretrained_model)
    
    # 3. 加载三国演义文本
    print("\n" + "="*60)
    print("加载三国演义文本")
    print("="*60)
    sanguoyanyi_path = "./sanguoyanyi.txt"
    sanguoyanyi_sentences = load_and_preprocess_data(sanguoyanyi_path)
    print(f"总共加载了 {len(sanguoyanyi_sentences)} 条三国演义文本数据")
    print("前5条数据示例：")
    for i in range(min(5, len(sanguoyanyi_sentences))):
        print(f"{i+1}: {sanguoyanyi_sentences[i][:50]}...")
    
    # 4. 对三国演义文本进行分词处理
    print("\n" + "="*60)
    print("对三国演义文本进行分词处理")
    print("="*60)
    tokenized_corpus = chinese_tokenize(sanguoyanyi_sentences)
    print("分词完成！")
    print("\n分词后的数据示例：")
    for i in range(min(3, len(tokenized_corpus))):
        print(f"原文: {sanguoyanyi_sentences[i][:30]}...")
    ...
    # 11. 保存词向量
    save_word_vectors_pt(word_vectors_dict, "sanguoyanyi_word_vectors.pt")
    
    # 12. 加载词向量
    loaded_word_vectors_dict = load_word_vectors_pt("sanguoyanyi_word_vectors.pt")
    
    # 13. 创建包装器
    w2v_model = PyTorchWord2VecWrapper(loaded_word_vectors_dict, word_to_idx, idx_to_word)
    
    # 14. 测试训练后的模型
    test_trained_model(w2v_model)
